describe('User Flow', () => {
  ...
  //   it('displays an error message when fetching a new image fails', () => {
  //     cy.intercept('GET', 'https://hargrimm-wikihow-v1.p.rapidapi.com/images?count=1', {
  //       statusCode: 500,
  //     }).as('fetchImageFail');
  //     cy.visit('/');
  //     cy.wait('@fetchImageFail');
  //     cy.get('.error-message', { timeout: 10000 }).should('contain', 'Failed to fetch new image. Please try again.');
  //     cy.get('.error-message').debug().should('contain', 'Failed to fetch new image. Please try again.');
